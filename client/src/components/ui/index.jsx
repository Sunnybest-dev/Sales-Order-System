...
export function Spinner({ size = 'md' }) {
  ...
  return (
    <div className={`${sizes[size]} border-2 border-gray-200 border-t-primary-600 rounded-full animate-spin shrink-0`} />
  );
}
...
export function Modal({ open, onClose, title, children, size = 'md' }) {
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-end sm:items-center justify-center">
      ...
      <div className={`relative bg-white w-full ${sizes[size]} sm:rounded-2xl rounded-t-2xl shadow-2xl max-h-[95vh] sm:max-h-[90vh] flex flex-col`}>
...
export function Pagination({ meta, onPageChange }) {
  ...
  const visible = pages.length <= 7 ? pages : pages.filter(p =>
    p === 1 || p === pages.length || Math.abs(p - meta.page) <= 1
  );
  return (
    <div className="flex flex-col sm:flex-row items-center justify-between gap-2 px-4 py-3 border-t border-gray-100">
      <p className="text-xs sm:text-sm text-gray-500 order-2 sm:order-1">
        {((meta.page - 1) * (meta.limit || 20)) + 1}–{Math.min(meta.page * (meta.limit || 20), meta.total)} of {meta.total}
      </p>
      <div className="flex gap-1 flex-wrap justify-center order-1 sm:order-2">
        ...
        {visible.map((p, i) => (
          <span key={p}>
            {i > 0 && visible[i - 1] !== p - 1 && <span className="w-8 h-8 flex items-center justify-center text-gray-400 text-sm">…</span>}
            <button
  ...
  return (
    <div className="card p-4 sm:p-5">
      <div className="flex items-start justify-between gap-2">
        ...
        <div className={`w-10 h-10 sm:w-12 sm:h-12 rounded-xl flex items-center justify-center text-lg sm:text-xl shrink-0 ${colors[color]}`}>
          {icon}
        </div>
      </div>
    </div>
