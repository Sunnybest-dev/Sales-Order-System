...
import { Button, Input, Select, Modal, Pagination, EmptyState, LoadingPage, PageHeader, ConfirmDialog } from '../../components/ui/index';
...
function ExpenseForm({ expense, onClose }) {
  ...
  const mutation = useMutation({
    ...
    onSuccess: () => { toast.success(expense ? 'Expense updated' : 'Expense recorded'); qc.invalidateQueries(['expenses']); onClose(); },
  });
  return (
    <form onSubmit={handleSubmit(d => mutation.mutate(d))} className="space-y-4">
      ...
      <div className="flex gap-3 justify-end pt-2">
        ...
        <Button type="submit" loading={mutation.isPending}>{expense ? 'Update' : 'Record'} Expense</Button>
      </div>
    </form>
  );
}
...
export default function ExpensesPage() {
  ...
  const deleteMutation = useMutation({
    ...
    onSuccess: () => { toast.success('Expense deleted'); qc.invalidateQueries(['expenses']); setDeleteTarget(null); },
  });
