...
export default function DashboardPage() {
  ...
  if (isError || !data) return <EmptyState title="Failed to load dashboard" description="Please refresh the page" />;
        ...
        <div className="card p-4 sm:p-5">
          ...
          <div className="h-48 sm:h-64">
            {topProducts.length > 0
              ? <Bar data={topProductsChart} options={CHART_OPTIONS} />
              ...
              {recentOrders.map((order) => (
                <div key={order.id} className="p-4 flex items-center justify-between gap-3">
                  <div className="min-w-0">
                    <Link to={`/orders/${order.id}`} className="text-primary-600 font-medium text-sm">
                      {order.order_number}
                    </Link>
        ...
        <div className="bg-red-50 border border-red-200 rounded-xl p-3 sm:p-4 flex flex-col sm:flex-row items-start sm:items-center gap-3">
          ...
          <div className="flex-1">
            <p className="font-semibold text-red-800 text-sm sm:text-base">
              {kpis.lowStockCount} product(s) are running low on stock
            </p>
            <p className="text-xs sm:text-sm text-red-600">Restock soon to avoid stockouts</p>
          </div>
          <Link
            to="/products"
            className="btn-danger text-sm px-4 py-2 rounded-lg shrink-0"
          >
            View Products
          </Link>
        </div>
