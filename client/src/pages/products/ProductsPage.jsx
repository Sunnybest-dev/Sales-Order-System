...
import { Button, Input, Select, Modal, Badge, Pagination, EmptyState, LoadingPage, PageHeader, ConfirmDialog } from '../../components/ui/index';
import { formatCurrency } from '../../utils/helpers';

function ProductForm({ product, categories, onClose }) {
  const qc = useQueryClient();
  const { register, handleSubmit, formState: { errors } } = useForm({ defaultValues: product || { min_stock_level: 10 } });
  const mutation = useMutation({
    mutationFn: (data) => product ? productsAPI.update(product.id, data) : productsAPI.create(data),
    onSuccess: () => { toast.success(product ? 'Product updated' : 'Product created'); qc.invalidateQueries(['products']); onClose(); },
  });
  return (
...
export default function ProductsPage() {
  ...
  const deleteMutation = useMutation({
    ...
    onSuccess: () => { toast.success('Product deactivated'); qc.invalidateQueries(['products']); setDeleteTarget(null); },
  });

  return (
    <div>
      <PageHeader title="Products & Inventory" subtitle={`${data?.meta?.total || 0} products`}
